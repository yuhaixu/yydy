...
// 用户自提
@Controller
@RequestMapping("/admin/zt_send_order")
public class UserZTSendOrderController extends BaseSecretController {
...
//        String oidStr = Utils.getSomeValue(request.getParameter("oid"));
        ...
        if (StringUtils.isNotBlank(jiuResp)) {
            ...
            responseBean = (ResponseBean) xStream.fromXML(jiuResp);
        ...
        if (responseBean != null) {
            if ("100000".equals(responseBean.getCode())) {
                ...
                if (low > 0) {
                    StOrdersBO ordersBO = new StOrdersBO();
                    ordersBO.setId((int)oid);
                    ordersBO.setIsZt(1);
    ...
    public String createSendOrder(long oid) {
        ...
        deliveryOrder.setLogisticsCode(JiuConfig.ZT_LOGISTICS_CODE);
